function dydt = fun_axial12nov19(~,y,mass,drag,ODm,holeD,Kc,rho,k,h1,n,ODj,h2,...
    ODc,hC,ODb,hB,aa,bb,Au1,Au2,AuC,AuB,F_hook,rNom)
v1 = [];
v2 = [];
Re1= [];
Re2= [];
...
Ff(end) = AuC*Tau1(end) + AuB*Tau2(end);%It should be different 

dydt =[ y(2)
    (F_hook)
    y(4)
    (rNom(2)*(y(1) -y(3)) - rNom(3)*(y(3)-y(5)) - ((drag(1)+Ff(1))/mass(2)))
    y(6)
    (rNom(4)*(y(3)-y(5)) - rNom(5)*(y(5)-y(7)) - ((drag(2)+Ff(2))/mass(3)))
    y(8)
    (rNom(6)*(y(5)-y(7)) - rNom(7)*(y(7)-y(9)) - ((drag(3)+Ff(3))/mass(4)))
    y(10)
    (rNom(8)*(y(7)-y(9)) - ((drag(4)+Ff(4))/mass(5)))];
